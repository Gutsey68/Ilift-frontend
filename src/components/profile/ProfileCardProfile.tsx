...
function ProfileCardProfile({ userDetails }: ProfileCardProps) {
  ...
  const [selectedFollowing, setSelectedFollowing] =
  ...
  return (
    <>
      <div className="relative flex items-center gap-4 border-b border-neutral-6 p-6 shadow-sm max-sm:flex-col max-sm:text-center">
        ...
        <Avatar
          src={userDetails.profilePhoto || "/uploads/profil.png"}
          alt={`Photo de ${userDetails.pseudo}`}
          className="mr-1"
          size="xl"
        />
        <div className="w-4/5">
          ...
          <div className="mt-2 flex items-center gap-6 text-sm text-neutral-10 max-sm:flex-col">
            ...
            <p>
              <CalendarDays size={16} className="mr-1 inline-block" />A rejoint
              Ilift{" "}
              {userDetails.createdAt
                ? formatRelativeTime(userDetails.createdAt)
                : ""}
            </p>
          </div>
          <div className="mt-2 flex items-center gap-6 text-sm text-neutral-10">
            <p
              ...
              className="flex cursor-pointer items-center gap-1 hover:text-green-9"
            >
              <span className="text-lg font-semibold text-green-9">
                {userDetails._count?.followedBy || 0}
              </span>
              abonnements
            </p>
            <p
            ...
            >
              <span className="text-lg font-semibold text-green-9">
                {userDetails._count?.following || 0}
              </span>
              abonnés
            </p>
            <p className="flex items-center gap-1">
              <span className="text-lg font-semibold text-green-9">
                {userDetails._count?.workouts || 0}
              </span>
              activités
            </p>
          </div>
      ...
      {selectedFollowing && (
        <UnfollowModal
          ...
          closeModal={() => setSelectedFollowing(null)}
        />
      )}
      {showFollowersModal && (
        <FollowersModal
